...
import home.example.board.service.search.MeiliSearchService;
...
public class SearchAPI {
    ...
    public ResponseEntity<JSONObject> meiliSearch(@io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    examples = {
                            @io.swagger.v3.oas.annotations.media.ExampleObject(
                                    ...
                                    value ="{ \"keyword\": \"검색어\", \"offset\": 0, \"limit\": 10, \"type\": \"all\"}"
                                                  ...
                                                  @RequestBody Map<String, Object> requestMap){
        log.info("MeiliSearchAPI - requestMap = {}", requestMap);
        String keyword = (String) requestMap.get("keyword");
        int offset = (int) requestMap.get("offset");
        int limit = (int) requestMap.get("limit");
        String type = (String) requestMap.get("type");
        ...
        result.put("type", type);
        try{
            result = meiliSearchService.search(keyword, offset, limit, type);
            return ResponseEntity.ok().body(result);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("MeiliSearchAPI - search error: {}", e.getMessage());
            result.put("error", "검색 중 오류가 발생했습니다.");
            return ResponseEntity.badRequest().body(result);
        }
    }
